...
/**
 * Adds Kiddies_Author_Info widget.
 */
class Kiddies_Author_Info extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
            'kiddies_author_info_widget',
            esc_html__( 'Kiddies: Sidebar Author Widget', 'kiddies' ),
            array( 'description' => esc_html__( 'Displays author short info.', 'kiddies' ), )
        );

        $this->social_networks = apply_filters( 'kiddies_author_widget_social_networks', array(
                'facebook', 
                'twitter',
                'linkedin',
                'instagram',
    ...
    public function widget( $args, $instance ) {
        ...
        do_action( 'kiddies_before_author_info');
        ?>
        <div class="theme-author-widget <?php echo esc_attr($style);?>">
            ...
            if($instance['author_img']){
                ...
                <div class="author-image">
                    <?php echo wp_get_attachment_image( absint($instance['author_img']), 'kiddies-boxed-image', "", array( "class" => "img-responsive" ) );  ?>
                </div>
                <?php
            }
            ?>
                    ...
                    if(!empty($social_networks) && is_array($social_networks)){
                        foreach($social_networks as $network){
                            if(!empty($instance[$network])){
                                $svg = Kiddies_SVG_Icons::get_theme_svg_name( $instance[$network] );
                                if($svg){
                                    ?>
        ...
        do_action( 'kiddies_after_author_info');
    ...
    public function form( $instance ) {
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('author_name')); ?>">
                <?php esc_attr_e('Author Name:', 'kiddies'); ?>
            </label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('author_desc')); ?>">
                <?php esc_attr_e('Short Description:', 'kiddies'); ?>
            </label>
        ...
        <div>
            <label for="<?php echo esc_attr( $this->get_field_id( 'author_img' ) ); ?>">
                <?php esc_attr_e('Author Image:', 'kiddies'); ?>
            </label>
            <?php
            ...
            <div class="image-field">
                ...
                <p>
                    ...
                    <button type="button" class="upload_image_button button" data-uploader-title-txt="<?php esc_attr_e( 'Use Image', 'kiddies' ); ?>" data-uploader-btn-txt="<?php esc_attr_e( 'Choose an Image', 'kiddies' ); ?>">
                        <?php _e( 'Upload/Add image', 'kiddies' ); ?>
                    </button>
                    <button type="button" class="remove_image_button button" style="<?php echo esc_attr($remove_button_style);?>"><?php _e( 'Remove image', 'kiddies' ); ?></button>
                </p>
            </div>
        </div>
        <div>
            <p>
                ...
                <label for="<?php echo esc_attr($this->get_field_id('image_border_radius')); ?>">
                    <?php _e('Enable Imaged Border Radius', 'kiddies'); ?>
                </label>
            </p>
        </div>
        <div>
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'style' ) ); ?>">
                    <span class="field-label"><?php _e('Style', 'kiddies'); ?></span>
                </label>
                <select id="<?php echo esc_attr( $this->get_field_id( 'style' ) ); ?>" name="<?php echo esc_attr($this->get_field_name('style')); ?>" class="widefat">
                    <option value="style_1" <?php selected('style_1', $style);?>><?php _e('Style 1', 'kiddies'); ?></option>
                    <option value="style_2" <?php selected('style_2', $style);?>><?php _e('Style 2', 'kiddies'); ?></option>
                    <option value="style_3" <?php selected('style_3', $style);?>><?php _e('Style 3', 'kiddies'); ?></option>
                </select>
            </p>
        </div>
        <?php
